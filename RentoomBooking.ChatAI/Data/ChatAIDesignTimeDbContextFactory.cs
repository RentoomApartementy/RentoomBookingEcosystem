...
public sealed class ChatAIDesignTimeDbContextFactory : IDesignTimeDbContextFactory<ChatAIDbContext>
{
    private static readonly string[] ConnectionStringKeys =
    {
        "POSTGRES_RENTOOM_BOOKING_DB_LOCAL",
        
        "CHAT_AI_DB_CONNECTION",
    ...
    public ChatAIDbContext CreateDbContext(string[] args)
    {
        var connectionString = ResolveConnectionString(args);
        if (string.IsNullOrWhiteSpace(connectionString))
        {
            throw new InvalidOperationException(
                "Unable to resolve ChatAIDbContext connection string. " +
                "Set environment variable POSTGRES_RENTOOM_BOOKING_DB_LOCAL or CHAT_AI_DB_CONNECTION, " +
                "or define ConnectionStrings.POSTGRES_RENTOOM_BOOKING_DB_LOCAL in Api/local.settings.json.");
        }

        var optionsBuilder = new DbContextOptionsBuilder<ChatAIDbContext>();
        optionsBuilder.UseNpgsql(connectionString);

        return new ChatAIDbContext(optionsBuilder.Options);
    }
    ...
    private static IEnumerable<string> GetSearchRoots()
    {
        ...
        var assemblyDirectory = Path.GetDirectoryName(typeof(ChatAIDesignTimeDbContextFactory).Assembly.Location);
    ...
    private static string? TryReadConnectionStringFromLocalSettings(string path)
    {
        ...
        {
            // Ignore malformed/foreign local.settings files and continue searching.
        }
